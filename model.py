...
class KT_backbone(nn.Module):
    def __init__(self, skill_dim, answer_dim, hidden_dim, output_dim, diff_dim):
        ...
        self.rnn = nn.LSTM(self.skill_dim+self.diff_dim+self.answer_dim, self.hidden_dim, batch_first=True)
        ...
        self.diff_emb = nn.Embedding(11+1, self.diff_dim)
    ...
    def forward(self, skill, answer, diff, perturbation=None):
        
        skill_embedding=self.skill_emb(skill)
        answer_embedding=self.answer_emb(answer)
        diff_embedding=self.diff_emb(diff)
        
        skill_embedding=torch.cat((skill_embedding,diff_embedding), 2)
